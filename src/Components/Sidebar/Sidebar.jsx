...
const Sidebar = () => {
  ...
  return (
    ...
    <div className="min-h-screen inline-flex flex-col bg-[#F0F4F9] justify-between py-[25px] px-[15px]">
      ...
      <div>
        <img
          className="w-5 block ml-[10px] "
          src={assets.menu_icon}
          alt=""
          onClick={() => setExtended((prev) => !prev)}
        />
        {/* newchat */}
        <div className="mt-[50px] inline-flex items-center gap-2.5 bg-[#e6eaf1] text-[12px] text-gray-900 cursor-pointer rounded-[50px] py-2.5 px-[15px]">
          <img className="w-5" src={assets.plus_icon} />
          {extended && <p className="">new chat</p>}
        </div>
        {/* recent */}
        {extended ? (
